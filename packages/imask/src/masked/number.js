import { escapeRegExp, DIRECTION } from '../core/utils';
import ChangeDetails from '../core/change-details';
import Masked from './base';
import IMask from '../core/holder';
/** Number mask */
export default class MaskedNumber extends Masked {
    ...
    doPrepareChar(ch, flags = {}) {
        ...
        */
        flags.input && flags.raw ||
            !flags.input && !flags.raw) ? ch.replace(this._mapToRadixRegExp, this.radix) : ch), flags);
        if (ch && !prepCh)
            details.skip = true;
    ...
    _separatorsCount(to, extendOnSeparators = false) {
        let count = 0;
    ...
    _separatorsCountFromSlice(slice = this._value) {
    ...
    doSkipInvalid(ch, flags = {}, checkTail) {
        const dropFractional = this.scale === 0 && ch !== this.thousandsSeparator && (ch === this.radix ||
    ...
    */
    ...
    typedValueEquals(value) {
        ...
        return (super.typedValueEquals(value) ||
            MaskedNumber.EMPTY_VALUES.includes(value) && MaskedNumber.EMPTY_VALUES.includes(this.typedValue)) && !(value === 0 && this.value === '');
    }
}
