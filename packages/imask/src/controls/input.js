import { objectIncludes, DIRECTION } from '../core/utils';
...
import HTMLInputMaskElement from './html-input-mask-element';
...
export default class InputMask {
    ...
    set mask(mask) {
        ...
        const masked = (mask instanceof IMask.Masked ? mask : createMask({ mask }));
    ...
    updateOptions(opts) {
        ...
        if (updateMask)
            this.mask = mask;
        if (updateOpts)
            this.masked.updateOptions(restOpts); // TODO
    ...
    _onInput(e) {
        ...
        const offset = this.masked.splice(details.startChangePos, details.removed.length, details.inserted, details.removeDirection, { input: true, raw: true }).offset;
